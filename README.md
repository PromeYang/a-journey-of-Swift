...
从来没有一门强类型的语言能像 `Swift` 这样, 在保证性能安全的前提下, 居然还能拥有脚本语言的灵活性.

从第一句 `"print("Hello, world!")"` 语句, 就让我们感受到那种满满的熟悉感, 忍不住想要去尝试走近新的世界, 因为它是这样的触手可及.

在与 `Swift` 的交往过程中, EX `JavaScript` 的语法很自然的过渡, 然而新语法又是那样鲜明的区别, 让我不会无所适从.

细心的同学可能已经发现了, 

在 `Swift` 中, 使用 `双引号` 包裹来表示字符和字符串, 语句的结尾不用加 `分号`

在 `JavaScript` 中, 约定俗成使用 `单引号` 包裹来表示字符和字符串, 语句的结尾加 `分号`

在 `Swift` 的世界里, 终于感受到了什么是线程安全, 什么是类型安全, 

我们开始去关注参数的类型, 返回值的类型, 考虑更多安全性以及稳定性的问题, 编写代码变得更加严谨, 可靠.

你会感受到, 当你的代码在编写的时候就几乎可以发现所有可能存在的问题, 

你会感受到, 当语法检测帮你排除手误操作, 友好的语法提示, 详尽的文档,

你会感受到, 当你再也不用麻木的修复bug, 快速敏捷的开发应用程序,
